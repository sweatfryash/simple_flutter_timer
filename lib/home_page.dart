...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  late Animation<int> _intAnim =
      IntTween(begin: 0, end: _lineNum).animate(_animationController);

  late AnimationController _animationController = AnimationController(
      duration: const Duration(milliseconds: 2000), vsync: this); //控制圈圈
  ...
  ///存放每次点击计次按钮时的[_currentDuration]
  ValueNotifier<Duration> _currentDuration =
      ValueNotifier(Duration.zero); //时刻增长着的，已经累计的时间
  late DateTime _startTime; //按下开始时
  late DateTime _pauseTime; //按下暂停时
  late DateTime _continueTime; //按下继续时
  final ScrollController _sc = ScrollController();
  late Ticker _ticker = createTicker(_onTick); //在此ticker的回调中触发计时功能
          ...
          headerSliverBuilder: (BuildContext context, bool innerBoxIsScrolled) {
            return <Widget>[
              SliverAppBar(
                ...
                flexibleSpace: ValueListenableBuilder<Duration>(
                    ...
                    child: Column(
                      children: [
                        Container(
                          ...
                          child: CustomPaint(
                            painter:
                                TimerPainter(_lineNum, _intAnim, _isActivated),
                            child: Column(
                    ...
                    builder:
                      ...
                      return NoScaleFlexibleSpaceBar(
                        ...
                        background: child ?? SizedBox.shrink(),
                      );
                    }),
              ),
            ];
  ...
  Widget _durationItem(int index) {
    String realIndex =
        (_durationList.length - index).toStringAsFixedFromStart(2);
    if (index == 0) {
      return Padding(
        ...
        child: Row(
          children: [
            ...
            ValueListenableBuilder(
              valueListenable: _currentDuration,
              builder: (BuildContext context, Duration currentDuration, _) {
                return Text(
                  '+${(currentDuration - _durationList.first).toStringAsMyFormat()}',
      ...
      return Padding(
        ...
        child: Row(
          children: [
            ...
            const Spacer(),
            Text(
                '+${(_durationList[index - 1] - _durationList[index]).toStringAsMyFormat()}',
                style: TextStyle(fontSize: 14))
          ],
        ),
      );
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: FlatButton(
                          ...
                          textColor:
                              _isAnimating ? _primaryColor : Colors.white,
                          shape: CircleBorder(
                              side: BorderSide(
                                  color: Colors.grey.shade300,
                                  width: _isAnimating ? 2 : 0)),
                          color:
                              _isAnimating ? Colors.transparent : _primaryColor,
  ...
  bool _handleScrollNotification(ScrollNotification notification) {
    if (notification is ScrollEndNotification) {
      if (_sc.offset > 0 &&
          _sc.offset < (_appbarExpandedHeight - _appbarHeight)) {
        double target =
            _isAppbarExpanded ? _appbarExpandedHeight - _appbarHeight : 0;
        Future.delayed(Duration.zero, () async {
          if (_runAnimateTo != null) {
            await _runAnimateTo;
          }
          _runAnimateTo = _sc.animateTo(target,
  ...
  double calculateTextWidth(
      String value, double fontSize, double maxWidth, int maxLines) {
